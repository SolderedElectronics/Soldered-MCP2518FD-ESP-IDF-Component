/**
 * @file mcp2518fd_dfs.h
 * @brief Register map, bit definitions and object types for the MCP2518FD CAN FD controller
 *
 * Ported from the Microchip MCP2518FD SPI driver (drv_canfdspi_register.h).
 * Register addresses, bitfield layouts and reset values are unchanged.
 *
 * @copyright Copyright (c) 2018 Microchip Technology Inc. and its subsidiaries.
 *            Redistributed under the terms described in the LICENSE file.
...
// *****************************************************************************
// *****************************************************************************
/* Driver return codes, returned by the mcp2518fd_* API calls */

#define CAN_OK             (0)    /**< Operation succeeded */
#define CAN_FAILINIT       (1)    /**< Controller initialization failed */
#define CAN_FAILTX         (2)    /**< Transmission failed */
#define CAN_MSGAVAIL       (3)    /**< A message is waiting in the RX FIFO */
#define CAN_NOMSG          (4)    /**< RX FIFO is empty */
#define CAN_CTRLERROR      (5)    /**< Controller reported an error */
#define CAN_GETTXBFTIMEOUT (6)    /**< Timed out waiting for a free TX buffer */
#define CAN_SENDMSGTIMEOUT (7)    /**< Timed out while sending */
#define CAN_FAIL           (0xff) /**< Generic failure */

// *****************************************************************************
/* Oscillator frequency populated on the breakout board */
...
typedef enum {
    ...
    MCP_8MHz,
    /* apply to MCP2518FD */
    MCP2518FD_40MHz = MCP_16MHz, /**< 40 MHz oscillator (kept at the MCP2515 value for compatibility) */
    MCP2518FD_20MHz,             /**< 20 MHz oscillator - what the Soldered breakout is populated with */
    MCP2518FD_10MHz,             /**< 10 MHz oscillator */
} MCP_CLOCK_T;

// *****************************************************************************
/* Classic CAN 2.0 bit rate presets, passed to mcp2518fd_begin() */
...
/**
 * @brief Build a CAN FD bit rate word from an arbitration bit rate and a data phase factor
 *
 * The data phase bit rate is `arbitration * factor`. A factor of 0 or 1 means no
 * bit rate switch, i.e. plain CAN 2.0 timing.
 */
#define MCP2518FD_BITRATE(factor, arbitration) ((((uint32_t)(factor)) << 24) | ((uint32_t)(arbitration) & 0xFFFFFUL))

#define CAN_125K_500K  MCP2518FD_BITRATE(4, 125000UL)   /**< 125 kbps arbitration, 500 kbps data */
#define CAN_250K_500K  MCP2518FD_BITRATE(2, 250000UL)   /**< 250 kbps arbitration, 500 kbps data */
#define CAN_250K_750K  MCP2518FD_BITRATE(3, 250000UL)   /**< 250 kbps arbitration, 750 kbps data */
#define CAN_250K_1M    MCP2518FD_BITRATE(4, 250000UL)   /**< 250 kbps arbitration, 1 Mbps data */
#define CAN_250K_1M5   MCP2518FD_BITRATE(6, 250000UL)   /**< 250 kbps arbitration, 1.5 Mbps data */
#define CAN_250K_2M    MCP2518FD_BITRATE(8, 250000UL)   /**< 250 kbps arbitration, 2 Mbps data */
#define CAN_250K_3M    MCP2518FD_BITRATE(12, 250000UL)  /**< 250 kbps arbitration, 3 Mbps data */
#define CAN_250K_4M    MCP2518FD_BITRATE(16, 250000UL)  /**< 250 kbps arbitration, 4 Mbps data */
#define CAN_500K_1M    MCP2518FD_BITRATE(2, 500000UL)   /**< 500 kbps arbitration, 1 Mbps data */
#define CAN_500K_2M    MCP2518FD_BITRATE(4, 500000UL)   /**< 500 kbps arbitration, 2 Mbps data */
#define CAN_500K_3M    MCP2518FD_BITRATE(6, 500000UL)   /**< 500 kbps arbitration, 3 Mbps data */
#define CAN_500K_4M    MCP2518FD_BITRATE(8, 500000UL)   /**< 500 kbps arbitration, 4 Mbps data */
#define CAN_500K_5M    MCP2518FD_BITRATE(10, 500000UL)  /**< 500 kbps arbitration, 5 Mbps data */
#define CAN_500K_6M5   MCP2518FD_BITRATE(13, 500000UL)  /**< 500 kbps arbitration, 6.5 Mbps data */
#define CAN_500K_8M    MCP2518FD_BITRATE(16, 500000UL)  /**< 500 kbps arbitration, 8 Mbps data */
#define CAN_500K_10M   MCP2518FD_BITRATE(20, 500000UL)  /**< 500 kbps arbitration, 10 Mbps data */
#define CAN_1000K_4M   MCP2518FD_BITRATE(4, 1000000UL)  /**< 1 Mbps arbitration, 4 Mbps data */
#define CAN_1000K_8M   MCP2518FD_BITRATE(8, 1000000UL)  /**< 1 Mbps arbitration, 8 Mbps data */
...
typedef enum { MCP2518FD_GPIO_PIN_0, MCP2518FD_GPIO_PIN_1 } GPIO_PIN_POS;

//! GPIO Pin Modes

typedef enum { MCP2518FD_GPIO_MODE_INT, MCP2518FD_GPIO_MODE_GPIO } GPIO_PIN_MODE;

//! GPIO Pin Directions

typedef enum { MCP2518FD_GPIO_OUTPUT, MCP2518FD_GPIO_INPUT } GPIO_PIN_DIRECTION;

//! GPIO Open Drain Mode

typedef enum { MCP2518FD_GPIO_PUSH_PULL, MCP2518FD_GPIO_OPEN_DRAIN } GPIO_OPEN_DRAIN_MODE;

//! GPIO Pin State

typedef enum { MCP2518FD_GPIO_LOW, MCP2518FD_GPIO_HIGH } GPIO_PIN_STATE;

//! Clock Output Mode

typedef enum { MCP2518FD_GPIO_CLKO_CLOCK, MCP2518FD_GPIO_CLKO_SOF } GPIO_CLKO_MODE;
